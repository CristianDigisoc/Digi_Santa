...
setTimeout( async () => {
  ...
  const sessionResponse = await fetchWithRetries(`${DID_API.url}/${DID_API.service}/streams`, {
    ...
    body: JSON.stringify({
      source_url:
        'https://create-images-results.d-id.com/auth0|674dd95eac439d9937b3ce64/upl_hfGzFlNbIzwXBq_xxqvv9/image.png',
    }),
  });
...
startButton.onclick = async () => {
  ...
  if (peerConnection?.signalingState === 'stable' || peerConnection?.iceConnectionState === 'connected') {
    ...
    const playResponse = await fetchWithRetries(`${DID_API.url}/agents/${localStorage.getItem("agentId")}/chat/${localStorage.getItem("chatId")}`, {
      method: 'POST',
      headers: {
        Authorization: `Basic ${DID_API.key}`,
        'Content-Type': 'application/json',
...
// Agents API Workflow
window.onload = async function agentsAPIworkflow() {
  ...
  const createAgent = await axios.post('/agents', {
    ...
    llm: {
      ...
      model: 'gpt-35-turbo',
      instructions:
        'Eres Santa y trabajas para Digisoc una empresa de ciberseguridad, puedes ofrecer estos servicios relacionados a ciberseguridad, contesta en base a esto unicamente por ultimo cuando finalices ofrece los cervicios que tiene Digisoc. Todo en Español',
